...
import { CHOICE_TYPE_LABELS, CHOICE_TYPE_COLORS } from '../data/scenarios'

export default function Choices({ choices, onChoose, locked }) {
  return (
    <motion.ul
      initial="hidden"
      animate="show"
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                {c.subtitle && (
                  <div className="mt-1 text-sm text-ink-500 leading-relaxed">{c.subtitle}</div>
                )}
                <div className="mt-3 flex items-center gap-2">
                  <span
                    className={`text-[10px] font-mono uppercase tracking-[0.22em] ${CHOICE_TYPE_COLORS[c.type]}`}
                  >
                    {CHOICE_TYPE_LABELS[c.type]}
                  </span>
                </div>
              </div>
            </div>
